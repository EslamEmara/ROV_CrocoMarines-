...
#we intialize the servo with duty cycle 7 to make it horizontally 90 degree then we can either increase it to 180 or decrease it to 0 degree
    ...
    def camup(self):
        global camera_pos
        if camera_pos <12 :
            self.servo.ChangeDutyCycle(camera_pos)                    #increasing the duty cycle of servo to raise the camera up
            time.sleep(0.5)
            print("camera is up to position ",camera_pos)
            camera_pos+=1
            self.servo.ChangeDutyCycle(0)                             #after setting the servo in required position we turn it off to 
    ...
    def camdown (self):
        global camera_pos
        if camera_pos>=2 :
            self.servo.ChangeDutyCycle(camera_pos)                   #decreasing the duty cycle of servo to lower the camera down
            time.sleep(0.5)
